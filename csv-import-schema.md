...
| `subscriptionPeriod` | `subscription period`, `subscriptionperiod` | **Fuzzy-matched against the fixed enum `Quarterly` / `Half-Annual` / `Annually`** (matches on prefix — `Quarter*`→`Quarterly`, `Half*`/`Semi*`→`Half-Annual`, `Annual*`→`Annually`, so cells like `Annual`, `Semi-Annual`, `Quarter` all resolve) — no longer arbitrary free text. Unmatched text is **dropped, not saved as raw text**. Only stored on new rows if `segment === 'Enterprise'`; otherwise stored as `''`. On an existing row it's backfilled only if currently blank, same "fill blanks only" rule as the rest of §4c — and only with a value that itself matches the enum. |
| `subscriptionDate` | `subscription date`, `subscriptiondate` | **Normalized to `yyyy-mm-dd`** (the format the date-picker in the UI requires) whenever the cell contains a 4-digit year and parses — `2026-01-01`, `Jan 1 2026`, `01/15/2026` all normalize fine. A year-less cell like `Aug 1` is **left as-is** rather than guessed: JS's date parser would otherwise silently resolve it to the year 2001, not the current year, so anything without an explicit 4-digit year skips normalization entirely — it still imports and shows correctly on the record's detail page, it just won't pre-populate the date-picker if the record is reopened for edit. Only stored on new rows if `segment === 'SaaS'`; otherwise stored as `''`. |

**Revenue note:** `value` on a Customer row is their **MRR**, not an annual figure. The Dashboard/Analytics tabs derive period-aware realized revenue from it as `value × subscriptionPeriod's months` (Enterprise: Quarterly=3, Half-Annual=6, Annually=12) or `value × 12` (SaaS, which has no fixed term) — that derived total is computed in the app, not a CSV column, and there's nothing to import/export for it directly.
...
- Customers: `sector` (field of work — free text) defaults to **`"General"`** if blank/unrecognized; `segment` (subscription model) is fuzzy-matched against `SaaS`/`Enterprise` and defaults to **`null`** if blank/unmatched (it's not actually known until Active Client — see §3) — an unrecognized value is never stored as-is; `package` defaults to `"—"`; `aiScore`/`referredBy` default to `null`; `leadSource` falls back to `source` then `"Import"`; `nextStep` defaults to the first configured next-step option; `subscriptionPeriod` is fuzzy-matched against `Quarterly`/`Half-Annual`/`Annually` (see §3) and `subscriptionDate` is normalized to `yyyy-mm-dd` when parseable, each defaulting to `''` and only one populated depending on `segment` (both stay `''` while `segment` is null).
...
7. **For Customers**, decide `Segment` per row (`Enterprise` vs `SaaS`) if you want `Subscription Period`/`Subscription Date` respected — only the matching one is stored. `Subscription Period` must read as `Quarterly`, `Half-Annual`, or `Annually` (or an obvious variant like `Annual`/`Semi-Annual`/`Quarter` — see §3); anything else is dropped rather than saved as raw text. `Subscription Date` should include a 4-digit year (e.g. `2026-01-15`) so it normalizes and pre-fills the date-picker on edit.
