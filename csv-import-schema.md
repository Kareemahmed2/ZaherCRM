...
| `segment` | `segment`, `customer type` | **The subscription/business model, `SaaS` or `Enterprise` — not an industry.** Fuzzy-matched (`matchInList`) against exactly those two values, same as `stage`/`category`; anything that doesn't match either is **not stored** (never saved as raw text). Defaults to `"Enterprise"` on brand-new companies when blank/unmatched. |
...
- Customers: `sector` (field of work — free text) defaults to **`"General"`** if blank/unrecognized; `segment` (subscription model) is fuzzy-matched against `SaaS`/`Enterprise` and defaults to **`"Enterprise"`** if blank/unmatched — an unrecognized value is never stored as-is; `package` defaults to `"—"`; `aiScore`/`referredBy` default to `null`; `leadSource` falls back to `source` then `"Import"`; `nextStep` defaults to the first configured next-step option; `subscriptionPeriod`/`subscriptionDate` default to `''` and only one is populated depending on `segment`.
